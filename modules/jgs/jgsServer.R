server_jgs <- function(input, output, session) {
  ...
  output$demo_jgs_fam <- DT::renderDataTable(
    DT::datatable(demo_gs_fam_dat,
      colnames = "",
      rownames = FALSE,
      options = list(
        lengthChange = FALSE,
  ...
  jgs_model_rhs <- reactive({
    comp <- c(
      paste0(input$jgs_fct, ifelse(is.null(input$jgs_fct), "", " (fix)")),
  ...
  jgs_gen_dat <- reactive({
    ...
    if (any(file_not_exist)) {
      validate(paste0(
        "[ERROR:] We cannot find some files (",
    ...
    for (i in 1:length(name)) {
      ...
      while (length(oneLine <- readLines(con, n = 1, warn = FALSE)) > 0) {
        ...
        if (str_detect(oneLine, "Error")) {
          validate(
            need(
              !str_detect(oneLine, "Error"),
              paste0(
                "[ERROR:] Plink detect an error for ",
  ...
  output$jgs_phe_summary <- renderUI({
    req(input$jgs_phe_upload)
    div(paste0(
      "There are ", nrow(unique(jgs_phe_dat())),
  ...
  output$jgs_phe_ped_summary <- renderUI({
    ...
    div(paste0(
      "There are ", sum(as.character(jgs_ped_dat()[[1]]) %in% as.character(jgs_phe_dat()[[1]])),
  ...
  output$jgs_gen_summary <- renderUI({
    req(input$jgs_gen_upload)
    div(
      paste0("There are ", sum(jgs_gen_dat()$summary$No_var)), "variants in ",
  ...
  output$jgs_phe_gen_summary <- renderUI({
    ...
    div(paste0(
      "There are ", sum(as.character(jgs_gen_dat()$ind) %in% as.character(jgs_phe_dat()[[1]])),
  ...
  output$jgs_ped_gen_summary <- renderUI({
    ...
    div(paste0(
      "There are ", sum(as.character(jgs_gen_dat()$ind) %in% as.character(jgs_ped_dat()[[1]])),
